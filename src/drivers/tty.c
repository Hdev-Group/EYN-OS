...
void scrollUp(uint8 lineNumber)
{
        string vidmem = (string)0xb8000;
        uint16 i = 0;
        clearLine(0,lineNumber-1);    // updated; i wanna kms
        for (i;i<sw*(sh-1)*2;i++)
        {
...
void printfch(char c)
{
    ...
    switch(c)
    {
        case (0x08):
                ...
                {
	                cursorX--;									
                        vidmem[(cursorY * sw + cursorX)*sd]=0;	 // color this color that, I HATE VIDEO MEMORY                        
	        }
	        break;
       /* case (0x09):                                // fuck meeeee
...
{
        uint16 i = 0;
        uint8 length = strlength(ch); // i dont wanna call the function 999999 times so i store length here
        for(i;i<length;i++)
        {
                printfch(ch[i]);
        }
...
void printf_gay(const char* format, ...)
{
	va_list ap;
	va_start(ap, format);
	uint8 *ptr;
	...
	for (ptr = format; *ptr != '\0'; ptr++) 
		...
		if (*ptr == '%') 
			...
			switch (*ptr) {
				case 's':
					printf_gay(va_arg(ap, uint8 *));
					break;
				//case 'c':
					//printf_gay(char_to_string(va_arg(ap, uint8 *)));
				case 'd':
					printf_gay(int_to_string(va_arg(ap, int)));
					break;
				case '%':
					printfch('%');
					break;
